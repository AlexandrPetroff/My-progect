...
const { createPayForm } = require('../assets/modules/Form');
...
test('Функция создания DOM-дерева должна вернуть DOM-элемент, в котором содержится строго четыре поля для ввода', () => {
  ...
  expect(form.form.querySelectorAll('input')[1].placeholder).toEqual('ММ/ГГ');
  expect(form.form.querySelectorAll('input')[2].placeholder).toEqual('CVC/CVV');
  expect(form.form.querySelectorAll('input')[3].placeholder).toEqual('E-mail');
});
