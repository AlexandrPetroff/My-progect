...
const currencyRateFeedGenerator = setInterval(() => {
	...
	const randomIndexFrom = Math.floor(Math.random() * currenciesLength)
	let randomIndexTo = Math.floor(Math.random() * currenciesLength)
	if (randomIndexFrom === randomIndexTo) {
		randomIndexTo = (randomIndexTo + 1) % currenciesLength
	}
	const from = KNOWN_CURRENCY_CODES[randomIndexFrom]
	const to = KNOWN_CURRENCY_CODES[randomIndexTo]
